    ...
    <div className="container mx-auto max-w-7xl">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {skillCategories.map((category, index) => (
          <Card 
            key={category.title} 
            className="bg-slate-800 border-slate-700 hover:border-blue-500 transition-all duration-300 animate-fade-in"
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill) => (
                  <Badge 
                    key={skill} 
                    variant="secondary" 
                    className="bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors cursor-default"
                  >
                    {skill}
                  </Badge>
                ))}
      ...
      <div className="mt-16 grid md:grid-cols-4 gap-8 text-center">
        ...
        <div className="space-y-2">
          ...
          <div className="text-slate-300">Models Deployed</div>
        </div>
        <div className="space-y-2">
          <div className="text-4xl font-bold text-orange-400">94%</div>
          <div className="text-slate-300">Avg Model Accuracy</div>
        </div>
      </div>
    </div>
  );
